...
SimpleSwitch::SimpleSwitch(bool enable_swap, port_t drop_port)
  : Switch(enable_swap),
    ...
    egress_buffers(SSWITCH_PRIORITY_QUEUEING_NB_PORT,
                   1024 /* normal capacity */,
                   100,
                   10,
  ...
  add_required_field("standard_metadata", "scheduler_id");
...
#ifdef SSWITCH_PRIORITY_QUEUEING_ON
    while (egress_buffers.push_front(i, 0, 0, nullptr) == 0) continue;
#else
...
int
SimpleSwitch::init_ats_queue(const bm::OptionsParser &parser) {
  Json::CharReaderBuilder builder;
  Json::Value root;
  std::ifstream in;
  std::string errs;
  std::string ats_file = parser.ats_file;
  ...
  in.open(ats_file);
    ...
    if (Json::parseFromStream(builder, in, &root, &errs)) {
      for (unsigned int i = 0; i < root["port"].size(); i++) {
        Json::Value j_port = root["port"][i];
        size_t port_id = j_port["id"].asInt();
        size_t number_of_queue = 0;

        for (unsigned int j = 0; j < j_port["scheduler_group"].size(); j++) {
          Json::Value j_group = j_port["scheduler_group"][j];
          size_t group_id = j_group["id"].asInt();
          size_t priority = j_group["priority"].asInt();
          size_t maximum_residence_time = j_group["maximum_residence_time"].asInt();
          auto &group = egress_buffers.create_group(port_id, group_id);
          
          group.scheduler_group_identifier = group_id;
          group.maximum_residence_time = maximum_residence_time;
          group.priority = priority;
          group.port = port_id;

          for (unsigned int k = 0; k < j_group["scheduler"].size(); k++) {
            Json::Value j_scheduler = j_group["scheduler"][k];
            size_t scheduler_id = j_scheduler["id"].asInt();
            size_t committed_burstsize = j_scheduler["committed_burstsize"].asInt();
            double committed_rate = j_scheduler["committed_rate"].asDouble();
            size_t capacity = j_scheduler["capacity"].asInt();

            auto &scheduler = egress_buffers.create_scheduler(port_id, scheduler_id);
            scheduler.scheduler_identifier = scheduler_id;
            scheduler.scheduler_group_identifier = group_id;
            scheduler.committed_burstsize_parameter = committed_burstsize;
            scheduler.committed_information_rate = committed_rate / 1000;
            scheduler.priority = priority;
            scheduler.capacity = capacity;
            scheduler.port = port_id;
            number_of_queue++;
          }
        }
        egress_buffers.create_queue(port_id, number_of_queue);
      }      
    }else {
...
void
SimpleSwitch::enqueue(port_t egress_port, size_t scheduler_id, std::unique_ptr<Packet> &&packet) {
    ...
    if (with_queueing_metadata) {
      ...
      phv->get_field("queueing_metadata.enq_qdepth")
          .set(egress_buffers.size(egress_port, scheduler_id));
    }
    ...
    auto size = packet->get_data_size();
    egress_buffers.push_front(
        egress_port, scheduler_id, size,
        std::move(packet));
    // if(ret==0)  std::cout<< "buffer_size:" <<egress_buffers.size(egress_port, scheduler_id) <<std::endl;
...
SimpleSwitch::multicast(Packet *packet, unsigned int mgid) {
  ...
  for (const auto &out : pre_out) {
    ...
    packet_copy->set_register(RegisterAccess::PACKET_LENGTH_REG_IDX,
                              packet_size);
    Field &scheduler_id = phv->get_field("standard_metadata.scheduler_id");
    auto id = scheduler_id.get_int();
    enqueue(egress_port, id, std::move(packet_copy));
  }
}
...
SimpleSwitch::ingress_thread() {
  ...
  while (1) {
    ...
    if (clone_mirror_session_id) {
      ...
      if (is_session_configured) {
        ...
        if (config.egress_port_valid) {
          ...
          Field &scheduler_id = phv->get_field("standard_metadata.scheduler_id");
          auto id = scheduler_id.get_int();
          enqueue(config.egress_port, id, std::move(packet_copy));
        }
    ...
    Field &scheduler_id = phv->get_field("standard_metadata.scheduler_id");
    auto id = scheduler_id.get_int();
    enqueue(egress_port, id, std::move(packet));
  }
}
...
SimpleSwitch::egress_thread(size_t worker_id) {
  ...
  while (1) {
    ...
    if (with_queueing_metadata) {
      auto id = phv->get_field("standard_metadata.scheduler_id").get_int();
      auto enq_timestamp =
      ...
      phv->get_field("queueing_metadata.deq_qdepth").set(
          egress_buffers.size(port, id));

      //std::cout<<phv->get_field("queueing_metadata.deq_timedelta")<<" "<<phv->get_field("queueing_metadata.deq_qdepth")<<std::endl;
