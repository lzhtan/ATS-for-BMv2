...
#define SSWITCH_PRIORITY_QUEUEING_NB_PORT 8
...
class SimpleSwitch : public Switch {
 ...
 public:
  ...
  int init_ats_queue(const OptionsParser &parser);
 ...
 private:
  static constexpr size_t nb_egress_threads = SSWITCH_PRIORITY_QUEUEING_NB_PORT;
  ...
  struct EgressThreadMapper {
    ...
    size_t operator()(size_t egress_port) {
      auto it = map.find(egress_port);
      if (it != map.end()) return it->second;
      auto p = map.emplace(egress_port, index);
      index++;
 ...
 private:
  ...
  void enqueue(port_t egress_port, size_t scheduler_id, std::unique_ptr<Packet> &&packet);
...
#ifdef SSWITCH_PRIORITY_QUEUEING_ON
  bm::QueueingLogicPriATS<std::unique_ptr<Packet>, EgressThreadMapper>
#else
