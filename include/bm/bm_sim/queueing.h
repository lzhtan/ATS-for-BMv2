...
class QueueingLogicPriATS {
 ...
 public:
  ...
  //! elements. The capacity of each priority queue can be changed later by
  //! using set_capacity(size_t queue_id, size_t priority, size_t c).
  QueueingLogicPriATS(size_t nb_workers, size_t capacity, size_t burst, size_t rate, FMap map_to_worker)
      : capacity(capacity),
        burst(burst),
        rate(rate),
        workers_info(nb_workers),
        scheduler_info_map(nb_workers),
        group_info_map(nb_workers),
        map_to_worker(std::move(map_to_worker)){ }
  ...
  //! are incorrect, an exception of type std::out_of_range will be thrown (same
  //! if the FMap object provided to the constructor does not behave correctly).
  int push_front(size_t egress_port, size_t scheduler_id, size_t packet_size, const T &item) {
    size_t worker_id = map_to_worker(egress_port);
    LockType lock(mutex);
    try{
      auto &scheduler_info = get_scheduler_info(egress_port, scheduler_id);
      auto &group_info = get_group_info(egress_port, scheduler_info.scheduler_group_identifier);
      auto &scheduler = workers_info.at(worker_id);
      if (scheduler_info.size >= scheduler_info.capacity) return 0;
      auto T_eligibility = get_next_tp(scheduler_info, group_info, clock::now(), packet_size);
      scheduler_info.last_sent = T_eligibility;
    
      scheduler.queues.at(scheduler_id).emplace(item, egress_port, scheduler_info.last_sent, scheduler.wrapping_counter++);
      scheduler_info.size++;
      group_info.size++;
      scheduler.size++;
      scheduler.q_not_empty.notify_one();
  ...
  //! Same as push_front(size_t queue_id, size_t priority, const T &item), but
  //! \p item is moved instead of copied.
  int push_front(size_t egress_port, size_t scheduler_id, size_t packet_size, T &&item) {
    size_t worker_id = map_to_worker(egress_port);
    LockType lock(mutex);
    try{
      auto &scheduler_info = get_scheduler_info(egress_port, scheduler_id);
      ...
      auto T_eligibility = get_next_tp(scheduler_info, group_info, clock::now(), packet_size);
      scheduler_info.last_sent = T_eligibility;
      scheduler.queues.at(scheduler_id).emplace(std::move(item), egress_port, scheduler_info.last_sent, scheduler.wrapping_counter++);
      scheduler_info.size++;
      group_info.size++;
      scheduler.size++;
      scheduler.q_not_empty.notify_one();
  ...
  void pop_back(size_t worker_id, size_t *queue_id, size_t *priority,
    ...
    while (true) {
      if (scheduler.size == 0) {
        scheduler.q_not_empty.wait(lock);
      } else {
        auto now = clock::now();
        auto next = clock::time_point::max();
        for (i = 0; i < scheduler.queues.size(); i++) {
          auto &q = scheduler.queues.at(i);
          if (q.size() == 0) continue;
          if (q.top().send <= now) {
            queue = &q;
            break;
        ...
        if (queue) break;
        scheduler.q_not_empty.wait_until(lock, next);
      }
    }
    *queue_id = queue->top().queue_id;
    auto &scheduler_info = get_scheduler_info(*queue_id, i);
    auto &group_info = get_group_info(*queue_id, scheduler_info.scheduler_group_identifier);
    *priority = scheduler_info.priority;
    // TODO(antonin): improve / document this
    // http://stackoverflow.com/questions/20149471/move-out-element-of-std-priority-queue-in-c11
    *pItem = std::move(const_cast<QE &>(queue->top()).e);
    queue->pop();
  ...
  //! The occupancies of all the priority queues for this logical queue are
  //! added.
  // size_t size(size_t scheduler) const {
  //   LockType lock(mutex);
  //   auto it = scheduler_info.find(scheduler);
  //   if (it == scheduler_info.end()) return 0;
  //   auto &q_info = it->second;
  //   return q_info.size;
  // }
  ...
  //! Get the occupancy of priority queue \p priority for logical queue with id
  //! \p group.
  size_t size(size_t egress_port, size_t scheduler_id) {
    LockType lock(mutex);
    try{
      auto &q_info = get_scheduler_info(egress_port, scheduler_id);
      return q_info.size;
    }catch(const char* msg){
      return 0;
    }
  ...
  //! Set the capacity of all the priority queues for logical queue \p egress_port
  //! to \p c elements.
  void set_capacity(size_t egress_port, size_t c) {
    LockType lock(mutex);
    for_each_q(egress_port, SetCapacityFn(c));
  }

  //! Set the capacity of scheduler queue \p scheduler for logical queue \p
  //! egress_port to \p c elements.
  void set_capacity(size_t egress_port, size_t scheduler, size_t c) {
    LockType lock(mutex);
    for_one_q(egress_port, scheduler, SetCapacityFn(c));
  }

  //! Set the capacity of all the scheduler queues of all logical queues to \p c
  //! elements.
  void set_capacity_for_all(size_t c) {
    LockType lock(mutex);
    for(auto& v : scheduler_info_map) {
      for(auto& u : v.scheduler_info) {
        u.second.capacity = c;
      }
    }
    capacity = c;
  ...
  //! Set the maximum rate of all the scheduler queues for logical queue \p
  //! egress_port to \p rate. \p rate is expressed in "number of elements per
  //! second". Until this function is called, there will be no rate limit for
  //! the queue. The same behavior (no rate limit) can be achieved by calling
  //! this method with a rate of 0.
  void set_rate(size_t egress_port, uint64_t rate) {
    LockType lock(mutex);
    for_each_q(egress_port, SetRateFn(rate));
  }

  //! Same as set_rate(size_t egress_port, uint64_t rate) but only applies to the
  //! given scheduler queue.
  void set_rate(size_t egress_port, size_t scheduler, uint64_t rate) {
    LockType lock(mutex);
    for_one_q(egress_port, scheduler, SetRateFn(rate));
  }

  //! Set the rate of all the scheduler queues of all logical queues to \p rate.
  void set_rate_for_all(uint64_t r) {
    LockType lock(mutex);
    for(auto& v : scheduler_info_map) {
      for(auto& u : v.scheduler_info) {
        u.second.committed_information_rate = r;
      }
    }
    rate = r;
  }

  void set_burst(size_t egress_port, uint64_t burst) {
    LockType lock(mutex);
    for_each_q(egress_port, SetBurstFn(burst));
  }

  //! Same as burst(size_t egress_port, uint64_t burst) but only applies to the
  //! given scheduler queue.
  void set_burst(size_t egress_port, size_t scheduler, uint64_t burst) {
    LockType lock(mutex);
    for_one_q(egress_port, scheduler, SetBurstFn(burst));
  }

  //! Set the burst of all the scheduler queues of all logical queues to \p burst.
  void set_burst_for_all(uint64_t b) {
    LockType lock(mutex);
    for(auto& v : scheduler_info_map) {
      for(auto& u : v.scheduler_info) {
        u.second.committed_burstsize_parameter = b;
      }
    }
    burst = b;
  }
  ...
  //! Deleted copy constructor
  QueueingLogicPriATS(const QueueingLogicPriATS &) = delete;
  //! Deleted copy assignment operator
  QueueingLogicPriATS &operator =(const QueueingLogicPriATS &) = delete;

  //! Deleted move constructor
  QueueingLogicPriATS(QueueingLogicPriATS &&) = delete;
  //! Deleted move assignment operator
  QueueingLogicPriATS &&operator =(QueueingLogicPriATS &&) = delete;
  ...
  using Scheduler = std::priority_queue<QE, std::deque<QE>, QEComp>;
  ...
  struct QueueInfo_Scheduler {
    QueueInfo_Scheduler(size_t si, size_t sgi, size_t cbp, double cir, size_t p, size_t c, size_t pt)
        : scheduler_identifier(si), 
          scheduler_group_identifier(sgi), 
          committed_burstsize_parameter(cbp), 
          committed_information_rate(cir),
          priority(p),
          capacity(c),
          port(pt),
          bucket_empty_time(clock::now()),
          last_sent(clock::now()) { }
    
    size_t scheduler_identifier;
    size_t scheduler_group_identifier;
    size_t committed_burstsize_parameter;
    double committed_information_rate;
    size_t priority;
    size_t size{0};
    size_t capacity;
    size_t port;
    clock::time_point bucket_empty_time;
    clock::time_point last_sent;
  };

  struct QueueInfo_Group {
    QueueInfo_Group(size_t sgi, size_t mrt, size_t p, size_t pt)
        : scheduler_group_identifier(sgi),
          maximum_residence_time(mrt),
          priority(p),
          port(pt),
          group_eligibility_time(clock::now()){ }
    
    size_t scheduler_group_identifier;
    size_t maximum_residence_time;
    size_t priority;
    size_t port;
    size_t size{0};
    clock::time_point group_eligibility_time;
  };
  
  clock::time_point get_next_tp(QueueInfo_Scheduler &scheduler, QueueInfo_Group &group, clock::time_point arrival_time, size_t packet_size) {
    clock::time_point T_shaper_eligibility = scheduler.bucket_empty_time +
              std::chrono::duration<size_t, std::nano>(static_cast<size_t>(packet_size * 8/ scheduler.committed_information_rate));

    clock::time_point T_bucket_full = scheduler.bucket_empty_time +
              std::chrono::duration<size_t, std::nano>(static_cast<size_t>(scheduler.committed_burstsize_parameter / scheduler.committed_information_rate));

    clock::time_point T_eligibility = std::max(std::max(T_shaper_eligibility, group.group_eligibility_time), arrival_time);

    if (T_eligibility <= (arrival_time + std::chrono::duration<size_t, std::nano>(group.maximum_residence_time))){
      group.group_eligibility_time = T_shaper_eligibility;
      scheduler.bucket_empty_time = (T_eligibility < T_bucket_full)?T_shaper_eligibility:(T_shaper_eligibility-T_bucket_full+T_eligibility);
      return T_eligibility; 
    }
  ...
  //! Create queue if the queue at port \p queue_id if the queue not exist.
  //! Otherwise retrun directly.
  QueueInfo_Group &create_group(size_t egress_port, size_t group_id) {
    auto &gp = group_info_map.at(egress_port).group_info;
    auto it = gp.find(group_id);
    if (it != gp.end()) return it->second;
    auto p = gp.emplace(
        group_id, QueueInfo_Group(group_id, 100, egress_port, 0));
    return p.first->second;
  }

  QueueInfo_Scheduler &create_scheduler(size_t egress_port, size_t scheduler_id) {
    auto &sp = scheduler_info_map.at(egress_port).scheduler_info;
    auto it = sp.find(scheduler_id);
    if (it != sp.end()) return it->second;
    auto p = sp.emplace(
        scheduler_id, QueueInfo_Scheduler(scheduler_id, 0, burst, rate, egress_port, capacity, 0));
    return p.first->second;
  }

  void create_queue(size_t port, size_t number) {
    size_t worker_id = map_to_worker(port);
    auto &worker = workers_info.at(worker_id);
    std::vector<Scheduler> q_t(number);
    worker.queues.swap(q_t);
  }
 
  QueueInfo_Scheduler &get_scheduler_info(size_t egress_port, size_t scheduler_id) {
    return scheduler_info_map.at(egress_port).scheduler_info.at(scheduler_id);
  }

  QueueInfo_Group &get_group_info(size_t egress_port, size_t group_id) {
    return group_info_map.at(egress_port).group_info.at(group_id);
  }

  template <typename Function>
  Function for_each_q(size_t egress_port, Function fn) {
    auto &sp = scheduler_info_map.at(egress_port).scheduler_info;
    for(auto &v : sp) {
      if(v.second.port == egress_port) {
        fn(v.second);
      }
    }
    return std::move(fn);
  ...
  template <typename Function>
  Function for_one_q(size_t egress_port, size_t scheduler, Function fn) {
    auto &sp = scheduler_info_map.at(egress_port).scheduler_info;
    for(auto &v : sp) {
      if(v.second.port == egress_port && v.second.scheduler_identifier == scheduler){
        fn(v.second);
        break;
      }
    }
  ...
  struct SetCapacityFn {
    ...
    void operator ()(QueueInfo_Scheduler &info) const {  // NOLINT(runtime/references)
      info.capacity = c;
    }
  ...
  struct SetRateFn {
    explicit SetRateFn(size_t rate)
        : rate(rate) {}

    void operator ()(QueueInfo_Scheduler &info) const {  // NOLINT(runtime/references)
      info.committed_information_rate = rate;
    }
  ...
  struct SetBurstFn {
    explicit SetBurstFn(size_t burst)
        : burst(burst) {}

    void operator ()(QueueInfo_Scheduler &info) const {  // NOLINT(runtime/references)
      info.committed_burstsize_parameter = burst;
    }
  ...
  struct Scheduler_map {
    std::unordered_map<size_t, QueueInfo_Scheduler> scheduler_info{};
  };

  struct Group_map {
    std::unordered_map<size_t, QueueInfo_Group> group_info{};
  };
  ...
  size_t rate;
  std::vector<WorkerInfo> workers_info{};
  std::vector<Scheduler_map> scheduler_info_map{};
  std::vector<Group_map> group_info_map{};
  FMap map_to_worker;
};
